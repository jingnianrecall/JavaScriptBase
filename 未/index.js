 ...
 * vue keep-alive
 * 
 * keep-alive是vue内置组件，作用是能够缓存不活动的组件
 * 
 * 组件切换时，默认会进行销毁（调用destroyed），如果想不销毁而是保存之前的状态，可以利用keep-alive来实现
 * 
 * keep-alive有两个属性：
 * include exclude 匹配的组件才会被缓存
 */
 ...
 * Vue生命周期
 * 
 * 1、Vue实例有一个完整的生命周期，从开始创建、初始化数据、编译模板、挂载DOM、
 * 渲染更新->渲染卸载一系列过程，称为Vue生命周期
 * 
 * 
 */
